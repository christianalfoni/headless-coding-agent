...
export class TogetherSDK {
     ...
     */
    parseFallbackResponse(content, availableTools) {
     ...
     */
    tryParseToolCall(text, availableTools) {
        ...
        try {
            ...
            if (parsed &&
                ...
                if (!toolExists && availableTools.length > 0) {
                    fs.appendFileSync("agent.log", `INVALID TOOL USAGE - Unknown tool: ${parsed.name}. Available tools: ${availableTools
                        .map((t) => t.name)
                        .join(", ")}\n`);
                    return null;
                }
            ...
            if (text.trim().includes('"name"') &&
                ...
                fs.appendFileSync("agent.log", `JSON PARSE FAILURE - Looks like incomplete tool call:\nOriginal: ${text}\nCleaned: ${cleanedText}\nError: ${error instanceof Error ? error.message : String(error)}\n`);
            }
        }
        return null;
    }
    async createChatCompletion(request) {
        try {
            ...
            if (request.tools && request.tools.length > 0) {
                ...
                const firstSystemMessageIndex = processedRequest.messages.findIndex((msg) => msg.role === "system");
...
- After receiving a tool result, you can call another tool if needed`;
                    processedRequest.messages[firstSystemMessageIndex] = {
                        ...processedRequest.messages[firstSystemMessageIndex],
                        content: processedRequest.messages[firstSystemMessageIndex].content +
            ...
            try {
                ...
                fs.appendFileSync("agent.log", `=== REQUEST MESSAGES ===\n${JSON.stringify(logMessages, null, 2)}\n========================\n`);
            }
            catch (error) {
            ...
            const usage = chatCompletion.usage
                ? {
                    ...
                    reasoning_tokens: chatCompletion.usage.reasoning_tokens || 0,
                }
                : undefined;
            // Log response for debugging
            try {
                fs.appendFileSync("agent.log", `=== RESPONSE DATA ===\n${JSON.stringify({
            ...
            parsedContent = this.parseFallbackResponse(messageContent, request.tools || []);
            return {
            ...
            fs.appendFileSync("agent.log", `=== SDK ERROR ===\n${error instanceof Error ? error.message : String(error)}\n=================\n`);
