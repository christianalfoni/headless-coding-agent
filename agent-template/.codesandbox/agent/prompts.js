export function createModels(provider, model, apiKey) {
    return {
        evaluateTodos: async ({ workspacePath, todos, prompt, todosContext, hasCompletedTodos, hasPendingTodos, projectAnalysis, }) => {
...
- index.html : The main entry file`;
        ...
        executeTodo: async ({ workspacePath, todo, todos, projectAnalysis, repos }) => {
            ...
            const remainingTodosContext = remainingPendingTodos.length > 0
            ...
            const allInteractedPaths = new Set();
            todos.forEach((t) => {
                if (t.paths) {
                    t.paths.forEach((path) => allInteractedPaths.add(path));
                }
            });
            const interactedPathsContext = allInteractedPaths.size > 0
                ? `\n\nFiles already interacted with in previous todos:\n${Array.from(allInteractedPaths)
                    .map((path) => `- ${path}`)
...
${repos.map((repo) => `- ${repo.folderName}: ${repo.remoteUrl}`).join('\n')}
...
This helps users understand exactly what was done and easily navigate to relevant code locations.`,
                prompt: `${todo.description}${todo.context ? `\n\nContext: ${todo.context}` : ""}`,
            };
        },
        summarizeTodos: async ({ workspacePath, todos }) => {
            const completedTodos = todos.filter((todo) => todo.status === "completed");
            const prompt = completedTodos.length > 0
