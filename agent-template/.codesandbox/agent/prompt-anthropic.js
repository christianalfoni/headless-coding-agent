...
export async function* streamPrompt(config) {
    try {
        ...
        const shouldUseThinking = config.reasoningEffort && config.reasoningEffort !== "low";
        ...
        while (true) {
            ...
            const thinkingBlocks = response.content.filter((content) => content.type === "thinking");
            ...
            if (hasToolCalls) {
                ...
                for (const toolUse of toolUses) {
                    ...
                    try {
                        // Execute the tool
                        const tool = Object.values(config.tools).find((t) => t.name === toolUse.name);
                        if (!tool) {
